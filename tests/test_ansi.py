...
def test_format_ansi_no_color():
    """Verify that format_ansi strips ANSI codes when color is disabled."""
    original_color = milc.ansi.ansi_config['color']
    try:
        milc.ansi.ansi_config['color'] = False
        string = '{fg_red}hello{style_reset_all}'
        result = milc.ansi.format_ansi(string)
        assert '\x1b[' not in result
        assert 'hello' in result
    finally:
        milc.ansi.ansi_config['color'] = original_color
